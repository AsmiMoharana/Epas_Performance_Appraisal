...
import java.sql.*;
class epas_RptAttendOutDeptWin implements ActionListener{
	...
	String colHead []={"Sl.No","Attnd Sl.No","Date","Staffid","Staff Name","Gender","Dept","Shift Name","End Time","Out Time","Early By"};
	Object data[][];
	JTable tblData;
	JScrollPane jspData;
	int slno1,rw;
	...
	public epas_RptAttendOutDeptWin(){
		f1=new JFrame("epas");
		p1=new JPanel();
		...
		lblHead=new JLabel("Attendance(Out) Information",JLabel.CENTER);
...
lblState.setFont(fnt2);
	...
	public void doConnect(){
	  ...
	  dept=dlg.showInputDialog(f1,"Enter Department","EPAS",1);
		rs=stmtView.executeQuery("select * from epas_tblAttendOut where dept='"+dept+"'");
		...
		while(rs.next()){
			...
			data[rw][8]=rs.getString("endtime");
			data[rw][9]=rs.getString("outtime");
			data[rw][10]=rs.getString("earlyby");
...
class epas_RptAttendOutDeptApp {
