...
import java.sql.*;
class epas_RptDepartmentWin implements ActionListener{
	...
	String colHead []={"Sl.No","Dept.Code","Dept Name","HOD","PhoneNo."};
	Object data[][];
	JTable tblData;
	JScrollPane jspData;
	int slno,rw;
	...
	public epas_RptDepartmentWin(){
		...
		lblHead=new JLabel("Department Information",JLabel.CENTER);
...
lblState.setFont(fnt2);
	...
	public void doConnect(){
	  ...
	  stmtView=conn.createStatement();
		rs=stmtView.executeQuery("select * from epas_tbldepartment");
		...
		while(rs.next()){
			data[rw][0]=slno;
			data[rw][1]=rs.getString("dcode");
			data[rw][2]=rs.getString("dname");
			data[rw][3]=rs.getString("hod");
			data[rw][4]=rs.getString("phoneno");
