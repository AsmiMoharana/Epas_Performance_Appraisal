...
import java.sql.*;
class epas_RptLeaveApproveStatusWin implements ActionListener{
	...
	String colHead []={"Sl.No","Staff Sl.No","Date","Reference No.","Staffid","Staff Name","Gender","Shift Name","Leave Type","From Date","To Date","Remark","Status"};
	Object data[][];
	JTable tblData;
	JScrollPane jspData;
	int slno1,rw;
	...
	public epas_RptLeaveApproveStatusWin(){
		f1=new JFrame("epas");
		p1=new JPanel();
		...
		lblHead=new JLabel("Leave Approve Information",JLabel.CENTER);
...
lblState.setFont(fnt2);
	...
	public void doConnect(){
	  ...
	  status=dlg.showInputDialog(f1,"Enter Status","EPAS",1);
		rs=stmtView.executeQuery("select * from epas_tblLeaveApproval where status='"+status+"'");
		
		rs.last();
		totrec=rs.getRow();
		rs.beforeFirst();
		data=new Object[totrec][13];
		...
		while(rs.next()){
			...
			data[rw][3]=rs.getString("refno");
			data[rw][4]=rs.getString("sid");
			data[rw][5]=rs.getString("sname");
			data[rw][6]=rs.getString("gender");
			data[rw][7]=rs.getString("sftname");
			data[rw][8]=rs.getString("leavetype");
			data[rw][9]=rs.getString("fromdate");
			data[rw][10]=rs.getString("todate");
			data[rw][11]=rs.getString("remark");
			data[rw][12]=rs.getString("status");
...
class epas_RptLeaveApproveStatusApp {
