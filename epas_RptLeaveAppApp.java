...
import java.sql.*;
class epas_RptLeaveAppWin implements ActionListener{
	...
	String colHead []={"Sl.No","Reference No.","Date","Staffid","Staff Name","Gender","Shift Name","Leave Type","From Date","To Date","Remark"};
	Object data[][];
	JTable tblData;
	JScrollPane jspData;
	int slno1,rw;
	...
	public epas_RptLeaveAppWin(){
		...
		lblHead=new JLabel("Leave Information",JLabel.CENTER);
...
lblState.setFont(fnt2);
	...
	public void doConnect(){
	  ...
	  stmtView=conn.createStatement();
		rs=stmtView.executeQuery("select * from epas_tblLeaveApp");
		...
		while(rs.next()){
			data[rw][0]=slno1;
			data[rw][1]=rs.getString("refno");
			data[rw][2]=rs.getString("date");
			data[rw][3]=rs.getString("sid");
			data[rw][4]=rs.getString("sname");
			data[rw][5]=rs.getString("gender");
			data[rw][6]=rs.getString("sftname");
			data[rw][7]=rs.getString("leavetype");
			data[rw][8]=rs.getString("fromdate");
			data[rw][9]=rs.getString("todate");
			data[rw][10]=rs.getString("remark");
