...
import java.sql.*;
class epas_RptIndusWin implements ActionListener{
	...
	String colHead []={"Sl.No","Regd.No","Industry Name","hname","locality","city","state","phoneno","emailid","gstno","gstdt"};
	Object data[][];
	JTable tblData;
	JScrollPane jspData;
	int slno,rw;
	...
	public epas_RptIndusWin(){
		...
		lblHead=new JLabel("Industry Information",JLabel.CENTER);
...
lblState.setFont(fnt2);
	...
	public void doConnect(){
	  ...
	  stmtView=conn.createStatement();
		rs=stmtView.executeQuery("select * from epas_tblIndus");
		...
		while(rs.next()){
			data[rw][0]=slno;
			data[rw][1]=rs.getString("regdno");
			data[rw][2]=rs.getString("iname");
			data[rw][3]=rs.getString("hname");
			data[rw][4]=rs.getString("locality");
			data[rw][5]=rs.getString("city");
			data[rw][6]=rs.getString("state");
			data[rw][7]=rs.getString("phoneno");
			data[rw][8]=rs.getString("emailid");
			data[rw][9]=rs.getString("gstno");
			data[rw][10]=rs.getString("gstdt");
			slno=slno+1;
			rw=rw+1;
	  }
	  }//try ends
